...
#breast_cancer <- read.csv(url("https://archive.ics.uci.edu/ml/machine-learning-databases/breast-cancer-wisconsin/wdbc.data"), header = FALSE)
...
prp(tennis_tree, type=2, extra=1, under=TRUE, compress=TRUE)
...
dev.off() # clear the format from the previinous plot
...
car.data = table(Cars93$AirBags, Cars93$EngineSize)
print(car.data)
...
#population with a finite level of variance, the mean of all samples from the same population will be approximately equal to the mean of the original population.
#Furthermore, the CLT states that as you increase the number of samples and the sample size
#the better the distribution of all of the sample means will approximate a normal distribution (a “bell curve“) even if the original variables themselves are not normally distributed.
